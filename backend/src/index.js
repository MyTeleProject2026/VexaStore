...
async function ensureReleaseMetadataSchema() {
  ...
  for (const statement of statements) { try { await pool.query(statement); } catch (error) { if (!String(error.message || '').includes('Duplicate column') && error.code !== 'ER_DUP_FIELDNAME') throw error; } }
}
