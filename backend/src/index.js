...
const allowedOrigins = new Set([
  ...
  'https://vexatrade-admin-n36m.onrender.com','https://admin.vexatrade-v.2bd.net',
  'https://vexatrade-6nhs.onrender.com','https://www.vexatrade-v.2bd.net',
  'https://learn-vexatrade.onrender.com','https://vexatrade.onrender.com',
  'https://vexatrade-admin.onrender.com','https://api-vexaaccount.onrender.com',
  'https://api-vexastore.onrender.com','https://vexatrade-5ycu.onrender.com',
  'https://vexatrade-ecosystem-api.onrender.com','https://vexatrade-server.onrender.com'
]);
...
app.get('/api', (req, res) => res.json({
  ...
  endpoints: {
    sso: ['/api/auth/vexaaccount/login','/api/auth/vexaaccount/start','/api/auth/vexaaccount/callback','/api/auth/vexaaccount/config-check'],
    release_management: ['/api/admin/release-versions'],
    platform_install: ['/api/platform/apps/:slug/install-manifest']
  }
}));
...
app.use((err, req, res, next) => {
  ...
  if (err.code === 'ER_DUP_ENTRY') return res.status(409).json({ success: false, message: 'Duplicate entry. This record already exists.' });
  if (err.code === 'ER_NO_REFERENCED_ROW') return res.status(400).json({ success: false, message: 'Invalid reference. The referenced record does not exist.' });
...
setInterval(async () => {
  try {
    ...
    connection.release();
    if (result.affectedRows > 0) console.log(`🧹 Cleaned up ${result.affectedRows} expired OTPs`);
  } catch (error) {
...
async function ensureAdminAccount() {
  ...
  const [adminRows] = await pool.query(
    'SELECT id, password FROM admin_users WHERE email = ? LIMIT 1',
    [adminEmail]
  );
  ...
  if (adminRows.length === 0) {
    ...
    await pool.query(
      `INSERT INTO admin_users (email, password, name, role, is_active)
       VALUES (?, ?, 'Administrator', 'super_admin', 1)`,
      [adminEmail, hashed]
    );
  ...
  // Older database migrations could contain an invalid/non-bcrypt placeholder
  // hash. Repair only malformed hashes; do not overwrite a valid admin password.
  const storedHash = adminRows[0].password;
  const isBcryptHash = typeof storedHash === 'string'
    && /^\$2[aby]\$\d{2}\$/.test(storedHash)
  ...
  if (!isBcryptHash) {
    ...
    await pool.query(
      ...
      [repairedHash, adminRows[0].id]
    );
    console.log(`🔧 Repaired invalid admin password hash for: ${adminEmail}`);
  }
}
...
async function startServer() {
  ...
  try {
    ...
    console.warn('⚠️ Startup database setup warning:', error.message);
  }
...
if (require.main === module) {
  startServer().catch(error => {
    console.error('❌ Failed to start VexaStore API:', error);
    process.exit(1);
  });
}
