...
function platformManifest(app, versions) {
  ...
  return {
    schema: 'vexastore-install-manifest-v2',
    ...
    webApp: web ? { url: web.file_url || app.website, versionId: web.id, version: web.version, installable: true, installMode: 'mtp2026-webapp-registry', pwaMode: 'browser-controlled', automaticInstallInsideMtp2026: true, automaticInstallOnPhysicalDevice: false } : null,
    nativePackages,
    supportedMtp2026Modes: ['ios', 'android', 'windows', 'gaming'],
    policy: {
      mtp2026: 'WebApps are installed into the authenticated MTP2026 application registry and become launchable from the MTP2026 Device OS shell.',
      ...
      ios: 'Native iOS packages require Apple-authorized signing/distribution. MTP2026 does not silently install arbitrary IPA files.',
...
router.post('/publish-web', authAdmin, async (req, res, next) => {
  ...
  try {
    ...
    res.status(201).json({ success: true, message: 'Web app published', data: { appId: appResult.insertId, versionId: versionResult.insertId, slug, installManifestPath: `/api/platform/apps/${slug}/install-manifest`, mtp2026InstallUrl: `https://mtp2026-app-launcher.onrender.com/?vexastoreInstall=1&slug=${encodeURIComponent(slug)}` } });
