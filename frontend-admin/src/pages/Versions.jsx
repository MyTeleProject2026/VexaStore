...
import { ArrowLeft, Plus, Trash2, Download, Smartphone, Apple, Monitor, Terminal, ShieldCheck, Globe2, ExternalLink } from 'lucide-react';

const OS_LABELS = { web: 'Web App / PWA', ios: 'iOS', android: 'Android', windows: 'Windows', macos: 'macOS', linux: 'Linux' };
const OS_ICONS = { web: Globe2, ios: Apple, android: Smartphone, windows: Monitor, macos: Apple, linux: Terminal };
...
export default function Versions() {
  ...
  const [form, setForm] = useState({ app_id: id, version: '', os: 'web', release_notes: '', is_latest: 1, file: null, web_url: '', sha256: '', package_name: '', version_code: '', minimum_sdk: '24', signing_certificate_sha256: '', release_status: 'PUBLISHED' });
  ...
  const resetForm = () => setForm({ app_id: id, version: '', os: 'web', release_notes: '', is_latest: 1, file: null, web_url: '', sha256: '', package_name: '', version_code: '', minimum_sdk: '24', signing_certificate_sha256: '', release_status: 'PUBLISHED' });

  const handleAddVersion = async (e) => {
    e.preventDefault();
    const isWeb = form.os === 'web';
    if (!form.app_id || !form.version.trim() || !form.os) { showError('Version and platform are required'); return; }
    if (isWeb) {
      try {
        const target = new URL(form.web_url.trim());
        if (target.protocol !== 'https:') throw new Error();
      } catch { showError('Web App releases require a valid HTTPS WebApp URL'); return; }
    } else if (!form.file) {
      showError('Native releases require an app file');
      return;
    }
    ...
    try {
      ...
      showSuccess(isWeb ? 'Web App release published.' : 'Release version published with verification metadata.');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-card p-6 flex flex-wrap items-center justify-between gap-4">
        <div><h1 className="text-2xl font-bold text-white">{app?.name || 'App'} — Versions</h1><p className="text-text-secondary">Manage WebApp/PWA and native releases, checksums, Android package identity, and publication state.</p></div>
      ...
      {showAddForm && (
        <form onSubmit={handleAddVersion} className="glass-card p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div><label className="input-label">Platform *</label><select value={form.os} onChange={(e) => setForm((p) => ({ ...p, os: e.target.value, file: null, sha256: '' }))} className="input-field"><option value="web">Web App / PWA</option><option value="android">Android (APK)</option><option value="ios">iOS (IPA)</option><option value="windows">Windows (EXE/MSI)</option><option value="macos">macOS (DMG)</option><option value="linux">Linux (DEB/RPM)</option></select></div>
          </div>
          {form.os === 'web' && <div className="p-4 rounded-xl border border-accent-primary/20 bg-accent-primary/5 space-y-3">
            <div><label className="input-label">Published WebApp URL *</label><input type="url" value={form.web_url} onChange={(e) => setForm((p) => ({ ...p, web_url: e.target.value }))} placeholder="https://your-webapp.example.com" className="input-field" required /></div>
            <p className="text-xs text-text-secondary">Use the public HTTPS URL of the application. If the target is a PWA, supported browsers can offer their normal installation flow; MTP2026/VexaStore never bypasses browser or OS installation security.</p>
          </div>}
          ...
          {form.os !== 'web' && <div><label className="input-label">App File *</label><input type="file" onChange={handleFileChange} className="w-full text-text-secondary file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:bg-accent-primary/10 file:text-accent-primary" required /><p className="text-xs text-text-secondary mt-1">APK, IPA, EXE, MSI, DMG, DEB, RPM, ZIP • SHA-256 is calculated automatically in your browser.</p></div>}
          <div className="flex items-center gap-2 text-xs text-text-secondary">{form.os === 'web' ? <><Globe2 size={15} /> WebApp release points to the HTTPS URL above.</> : hashing ? <><ShieldCheck size={15} className="animate-pulse" /> Calculating SHA-256…</> : form.sha256 ? <><ShieldCheck size={15} /> SHA-256: <span className="font-mono break-all">{form.sha256}</span></> : 'Select a file to calculate its SHA-256.'}</div>
